...
in {
  ...
  home.file.".config/eww/eww.yuck".text = builtins.replaceStrings ["$$"] ["$"] ''
    ...
    (defpoll time :interval "1s"
      "${pkgs.busybox}/bin/date '+%m月%d日 %H:%M:%S'")
		...
		(defpoll CPU_USAGE :run-while control-center-enable :interval "1s"  "~/.config/eww/scripts/sys_info --cpu")
		(defpoll MEM_USAGE :run-while control-center-enable :interval "1s"  "~/.config/eww/scripts/sys_info --mem")
		(defpoll BLIGHT :run-while control-center-enable :interval "1s"  "~/.config/eww/scripts/sys_info --blight")
		(defpoll BATTERY :run-while control-center-enable :interval "5s"  "~/.config/eww/scripts/sys_info --bat")
    ...
    (deflisten SONG :interval ""  "playerctl metadata --format '{{ title }}';playerctl --follow metadata --format '{{ title }}'")
		...
		(defpoll CTIME :run-while control-center-enable :interval "1s"  "~/.config/eww/scripts/music_info --ctime")
		...
		(defwidget logout [] 
			(box :class "genwin" :vexpand "false" :hexpand "false" 
				(button :class "btn_logout" :onclick "betterlockscreen -l blur & /run/current-system/sw/bin/systemctl suspend && fg" "﫼")))
		(defwidget sleep [] 
			(box :class "genwin" :vexpand "false" :hexpand "false" 
				(button :class "btn_sleep" :onclick "eww set control-center-enable=false; betterlockscreen -l blur && amixer set Master 0%" "⏾")))
		(defwidget reboot [] 
			(box :class "genwin" :vexpand "false" :hexpand "false" 
      ...
      (defwidget bar-music []
        (box :class "bar-music"
             ...
             :space-evenly false
             :valign "center"
             (label :limit-width 20 :text "$${SONG} $${ARTIST}")))
