...
import type { NeovimConfig } from './types.js'

export type { NeovimConfig }

export const DEFAULT_LUA_MODULE = 'core.agent'

export function resolveConfig(raw: Record<string, unknown> = {}): NeovimConfig {
  const luaModule =
    raw.luaModule === undefined || raw.luaModule === null
      ? DEFAULT_LUA_MODULE
  ...
  const socket =
    raw.socket === undefined || raw.socket === null
      ? ''
 ...
 */
export function resolveSocket(config: NeovimConfig): string | null {
  if (config.socket) return config.socket
  if (process.env.NVIM_LISTEN_ADDRESS) return process.env.NVIM_LISTEN_ADDRESS
