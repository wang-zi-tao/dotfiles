...
import type { DshContext, LoggerLike, ToolDefinition, ToolRunContext } from './types.js'
...
interface ToolSpec {
  ...
  run(args: any, exec: ToolRunContext): Promise<string>
}
...
export function apply(ctx: DshContext, rawConfig: Record<string, unknown> = {}): void {
  ...
  const specs: ToolSpec[] = [
    {
      ...
      async run(args) {
        const nv = await ensureNvim()
        const ret = await nv.command(args.cmd as string)
    ...
    },
    {
      ...
      async run(args) {
        const nv = await ensureNvim()
        const ret = await nv.luaEval(args.cmd as string)
    ...
    },
    {
      ...
      async run(args, exec) {
        ...
        return (
          `调试会话已启动: **${ret.config_name || args.config_name}** (${ret.lang || args.lang || 'cpp'})\n` +
          `- request: ${cfg.request || '?'}\n` +
          `- type: ${cfg.type || '?'}\n` +
    ...
    },
    {
      ...
      async run(args, exec) {
        ...
        if (ret.condition) out += ` (条件: ${ret.condition})`
        return out
      },
    },
    {
  ...
  // ------------------------------------------------------------------
  ctx.on('tools/result', (exec: { name?: string; arguments?: unknown }) => {
