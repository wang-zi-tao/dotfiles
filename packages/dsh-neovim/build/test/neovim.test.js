 ...
 */
const CORE_AGENT_STUB = 'package.loaded["core.agent"] = { run_async = function(fn, channel_id, task_id) ' +
...
test('run neovim command lua error', async (t) => {
    ...
    if (!nv)
        return;
    await assert.rejects(() => nv.luaAsyncEval("error('error message')", 'core.agent'), (error) => error !== null && error !== undefined);
});
