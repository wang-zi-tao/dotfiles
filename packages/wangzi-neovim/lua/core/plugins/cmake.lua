...
require('cmake').setup({
  ...
  samples_path = (n.core .. '/cmake_samples'), -- Folder with samples. `samples` folder from the plugin directory is used by default.
  default_projects_path = tostring(Path:new(vim.loop.os_homedir(), 'Projects')), -- Default folder for creating project.
  configure_args = { '-D', 'CMAKE_CXX_COMPILER=clang++', '-D', 'CMAKE_C_COMPILER=clang', '-D', 'CMAKE_CXX_FLAGS_DEBUG=-static -O0 -Wall -g2 -ggdb' }, -- Default arguments that will be always passed at cmake configure step. By default tells cmake to generate `compile_commands.json`.
