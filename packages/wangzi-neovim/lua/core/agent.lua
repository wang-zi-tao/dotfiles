...
local M = require("core.hotreload").module_export(function()
  return {
    nextId = 0,
    disableSessionEvent = {}
  }
...
---@param opts? {timeout_ms?: integer} -- 等待命中的超时毫秒数（默认 30000）
---@return table
...
--- 轮询等待会话停止（帧名变化）。与 step_and_wait 不同：不依赖协程，
--- 可在普通调用中使用。基线为当前帧名，帧名变化即视为停止。
---@param opts? {timeout_ms?: integer, poll_ms?: integer} -- timeout_ms 超时毫秒数（默认 30000），poll_ms 轮询间隔毫秒数（默认 200）
---@return {status: "stopped"|"timeout", thread_id?: integer, frame_name?: string, frame?: table}
function M.dap_wait_stop(opts)
  ...
  while hrtime() < deadline do
    ...
    if frame and frame.name ~= baseline then
      return {
        status = "stopped",
        thread_id = session.stopped_thread_id,
        frame_name = frame.name,
        frame = M.clean_table(frame),
      }
    end
  end
...
---@param frame_id integer -- 帧 ID（来自 stackTrace，如 nvim_dap_get_stack 的 frames[].id）
---@param reg_name string -- 寄存器名（大写，如 "RSP"/"RIP"/"RAX"/"RDI"/"RBP"）
---@return string? -- 寄存器值字符串，未找到返回 nil
function M.dap_read_register(frame_id, reg_name)
  local session = M.get_session()
  if not frame_id or not reg_name then
    return nil
  end
  ...
  if not cpu_group or not cpu_group.variablesReference then
    return nil
  end
