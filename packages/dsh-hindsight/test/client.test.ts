...
function jsonFetch(response: MockResponse): typeof globalThis.fetch {
  return (async () => ({
    ok: response.ok,
...
test('retain posts to the bank memories endpoint with bearer auth', async () => {
  ...
  const client = new HindsightClient({
    ...
    fetchImpl: async (url, init) => {
      ...
      return { ok: true, status: 200, statusText: '', text: async () => JSON.stringify({ success: true, items_count: 1 }) } as Response
    },
  })
...
test('non-2xx responses throw HindsightError with status', async () => {
  ...
  await assert.rejects(
    () => client.version(),
    (error: unknown) => error instanceof HindsightError && error.status === 401 && /bad key/.test(error.message),
  )
})
