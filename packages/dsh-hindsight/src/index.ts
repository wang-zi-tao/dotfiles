...
function optionalInteger(args: any, key: string, fallback: number | undefined, minimum = 1): number | undefined {
  const value = args?.[key]
...
function optionalBudget(args: any, fallback: string): string {
...
function optionalTags(args: any, fallback: string[]): string[] {
  const value = args?.tags ?? fallback
  if (value === undefined || value === null) return []
  return mergeTags(Array.isArray(value) ? value : String(value).split(','))
}

function optionalTagsMatch(args: any, fallback: string): string {
...
function optionalTypes(args: any, fallback: string[]): string[] {
...
export function apply(ctx: DshContext, rawConfig: HindsightConfig | Record<string, unknown> = {}): void {
  ...
  function handleCompletedTurn(session: SessionLike, event: SessionEventLike): void {
    ...
    state.chain = state.chain
      .then(async () => {
        ...
        if (record.query) await refreshRecall(session, record.query)
      })
  ...
  ctx.systemPrompt.context({
    ...
    text: (assemblyContext) => {
      if (config.memoryMode === 'tools') return ''
