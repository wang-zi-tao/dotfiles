...
function toolResultText(content) {
    ...
    for (const block of content) {
        ...
        if (block.type === 'tool-result' && Array.isArray(block.content))
            parts.push(textOfBlocks(block.content));
    }
 ...
 */
export function buildTurnRecord(session, turn, config) {
...
export function turnQuery(session, turn, config) {
    const events = eventsForTurn(session?.events ?? [], turn);
    for (const event of events) {
        ...
        if (text)
            return clip(text, config.recallMaxInputChars);
    }
    return '';
}
