...
export function isUriInside(uri, rootUri) {
    if (!isFileUri(uri) || !isFileUri(rootUri))
        return false;
 ...
 * inside it.
 */
export function toLspLocation(rawUri, rawRange, rootUri) {
    let path;
    if (isFileUri(rawUri)) {
...
export function toSymbolEntry(raw, rootUri) {
    let kind;
    if (typeof raw.kind === 'number')
        kind = SymbolKindName[raw.kind] ?? String(raw.kind);
    else
        kind = raw.kind;
    const entry = {
        name: raw.name,
    ...
    if (raw.containerName)
        entry.containerName = raw.containerName;
    return entry;
}
 ...
 */
export function readLinePreview(location) {
    try {
        if (!isFileUri(location.uri))
            return '';
 ...
 */
export function renderLocations(title, locations, maxLocations, withPreview = false) {
