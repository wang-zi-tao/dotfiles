...
import { pathToFileUri, readLinePreview, renderLocations, toLspLocation, toSymbolEntry } from './protocol.js';
...
function optionalInteger(args, key, minimum = 1) {
    const value = args?.[key];
...
function renderSymbols(symbols, maxLocations, withPreview = false) {
    ...
    if (truncated)
        lines.push(`… ${symbols.length - maxLocations} more symbol(s) omitted (maxLocations=${maxLocations})`);
    return { text: lines.join('\n'), truncated };
}
/**
 * Run one LSP operation and return the canonical structured result.
 */
async function runQuery(registry, args, config, exec) {
    ...
    if (args.operation === 'workspaceSymbol') {
        ...
        const symbols = (raw ?? []).map(sym => toSymbolEntry({
            name: sym.name,
            kind: sym.kind,
    ...
    const { client, root } = await registry.resolve(filePath, cwd, signal);
    const rootUri = pathToFileUri(root);
    const toLocations = (raw) => raw.map(loc => toLspLocation(loc.uri, loc.range, rootUri));
    switch (args.operation) {
        ...
        case 'explore': {
            ...
            try {
                ...
                symbolName = findName(docSymbols);
            }
            catch {
...
function renderResult(args, value, config) {
    switch (value.kind) {
        case 'locations': {
        ...
        case 'diagnostics':
            ...
            return value.diagnostics.map((d) => `[${d.severity}] ${d.source ? d.source + ': ' : ''}${d.range.startLine}:${d.range.startCharacter} ${d.message}`).join('\n');
        case 'explore': {
            const e = value.explore;
            const lines = [];
            if (e.symbolName)
                lines.push(`symbol: ${e.symbolName}`);
            if (e.hover)
                lines.push(`\n${e.hover}`);
            lines.push('');
            ...
            try {
                switch (effectiveVerb) {
                    case 'status': {
                        ...
                        if (rows.length === 0)
                            return { kind: 'success', text: 'No LSP servers configured.' };
