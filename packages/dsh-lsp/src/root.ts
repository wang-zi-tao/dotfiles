 ...
 */
function findMarker(dir: string, markers: readonly string[]): string | null {
 ...
 */
export function findRoot(startDir: string, spec: ServerSpec): string {
  ...
  for (;;) {
    ...
    if (vcsRoot === null) {
      if (existsSync(join(current, '.git'))) vcsRoot = current
      else if (existsSync(join(current, '.hg'))) vcsRoot = current
    }
...
export class RootResolver {
  ...
  resolve(startDir: string, spec: ServerSpec): string {
