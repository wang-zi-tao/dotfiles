...
local cpu_usage_on_panel = process(
    "cpu_usage",
...
local ram_usage_on_panel = process(
    "ram_usage",
    [[<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
local hdd_usage_on_panel = process(
    "hdd_usage",
    [[<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
local temp_status_on_panel = process(
    "temp_status",
    [[<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
function M.panel(screen)
    ...
    watch([[bash -c " cat /proc/stat | grep '^cpu ' "]], 1, function(_, stdout)
        data.user, data.nice, data.system, data.idle, data.iowait, data.irq, data.softirq, data.steal, data.guest, data.guest_nice =
