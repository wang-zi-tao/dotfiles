...
in {
  ...
  config = {
    networking.firewall.trustedInterfaces = mkIfWireguard [ "wg0" "wg1" ];
    lazyPackage = with pkgs; mkIfWireguard [ wireguard-tools ];
    networking.hosts = mkIfWireguard (listToAttrs (concatLists (mapAttrsToList
      (nodeName: node:
        let nodeWireguard = wireguardCluster.${nodeName};
        in optionals (hasAttr nodeName wireguardCluster) [
          {
    ...
    networking.firewall.allowedUDPPorts =
      mkIfWireguard [ wireguard.config.port (wireguard.config.port + 1) ];
    ...
    boot.kernel.sysctl = {
      "net.ipv4.conf.${wireguard.config.nat}.route_localnet" = 1;
    };

    networking.nat = mkIf (enabled && wireguard.config.iptables.enable) {
      enable = true;
      enableIPv6 = true;
      externalInterface = wireguard.config.nat;
    ...
    networking.wireguard = optionalAttrs enabled {
      ...
      interfaces = {
        wg0 = {
          ...
          mtu = 1200;
          peers = mapAttrsToList (otherNodeName: edge:
            let
              otherNodeNetworkConfig = networkCluster.${otherNodeName}.config;
              otherNodeWireguardConfig =
                wireguardCluster.${otherNodeName}.config;
            in {
              persistentKeepalive =
                if network.config.publicIp == null then 32 else 0;
              inherit (otherNodeWireguardConfig) publicKey;
              endpoint = if edge.tunnel then
                "127.0.0.1:${toString (otherNodeWireguardConfig.index + 40000)}"
              else if otherNodeNetworkConfig.publicIp != null then
                "${otherNodeNetworkConfig.publicIp}:${
                  toString otherNodeWireguardConfig.port
                }"
              else if otherNodeNetworkConfig.localIp != null then
                "${otherNodeNetworkConfig.localIp}:${
                  toString otherNodeWireguardConfig.port
                }"
              else
                null;
              allowedIPs = [
                ...
                (concatLists (mapAttrsToList (nodeName: node:
    ...
    sops.secrets."wg-shared-keys" =
      mkIfWireguard { sopsFile = config.cluster.keys.wireguard.sharedKeySops; };
    ...
    environment.etc."wg_netmanager/network.yaml" = mkIfWireguard {
      text = lib.generators.toYAML { } {
        ...
        peers = concatLists (mapAttrsToList (name: wireguardNode:
          let networkNodeConfig = networkCluster.${name}.config;
          in optional (networkNodeConfig.publicIp != null) {
            endPoint = "${networkNodeConfig.publicIp}:${
                toString (wireguardNode.config.port + 1)
              }";
    ...
    networking.firewall.allowedTCPPorts = mkIfWireguard ([ 52343 ]
      ++ concatLists (mapAttrsToList (otherNodeName: edge:
        (optional edge.tunnel
          (wireguardCluster.${otherNodeName}.config.index + 40000)))
        wireguard.peers));
    systemd.services = mkIfWireguard (listToAttrs ((optionals enabled [
      (nameValuePair "wg-netmanager" {
        enable = true;
        description = "Wireguard network manager wg1";
        wantedBy = [ "multi-user.target" ];
        after = [ "network.target" ];
        path = with pkgs; [ wg-netmanager jq wireguard-tools iproute2 ];
        script = ''
          ...
          wg_netmanager ${
            if (network.config.publicIp == null) then
              "-H"
            else
              "-w ${toString (wireguard.config.port + 1)}"
          } -c $TMPFILE
        '';
        serviceConfig = {
          Type = "simple";
    ...
    ]) ++ (concatLists (mapAttrsToList (otherNodeName: value:
      optional
      (value.tunnel && networkCluster.${otherNodeName}.config.publicIp != null)
      (nameValuePair "wireguard-tunnel-client-${otherNodeName}" {
        ...
        serviceConfig = {
          ...
          RestartSec = "5s";
          ExecStart =
            "${pkgs.udp2raw}/bin/udp2raw --fix-gro -k qMQ9rUOA --raw-mode faketcp --cipher-mode xor --auth-mode simple -c -l127.0.0.1:${
      ...
      })) wireguard.peers)) ++ (concatLists (mapAttrsToList
        (otherNodeName: value:
          ...
          (nameValuePair "wireguard-tunnel-server-${otherNodeName}" {
            ...
            serviceConfig = {
              ...
              RestartSec = "5s";
              ExecStart =
                "${pkgs.udp2raw}/bin/udp2raw --fix-gro -k qMQ9rUOA --raw-mode faketcp --cipher-mode xor --auth-mode simple -s -l0.0.0.0:${
                  toString
                  (wireguardCluster.${otherNodeName}.config.index + 40000)
