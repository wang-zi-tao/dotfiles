...
with builtins; with lib;with lib.types; with lib.attrsets; let
  ...
  wireguardGraph = pkgs.graphType {
    nodeOption = { nodeName, nodeConfig, ... }: {
      ...
      port = mkOption { type = nullOr ints.u16; default = null; };
...
in
{
  ...
  config = {
    ...
    networking.hosts = mkIfWireguard (listToAttrs
      (concatLists
        (mapAttrsToList
          (nodeName: node:
            let
    ...
    networking.firewall.allowedUDPPorts = mkIfWireguard [ wireguard.config.port (wireguard.config.port + 1) ];
